...
void settings_restore(uint8_t restore_flag)
{
  if (restore_flag & SETTINGS_RESTORE_DEFAULTS)
  {
    ...
    settings.myflags = 0;
    if (DEFAULT_SERVO_MODE)
    {
      settings.myflags |= BITFLAG_SERVO_MODE;
    }
    if (DEFAULT_FLAG_1)
    {
      settings.myflags |= BITFLAG_1;
    }
    if (DEFAULT_FLAG_2)
    {
      settings.myflags |= BITFLAG_2;
    }
    if (DEFAULT_FLAG_3)
    {
      settings.myflags |= BITFLAG_3;
    }
    if (DEFAULT_FLAG_4)
    {
      settings.myflags |= BITFLAG_4;
    }
    if (DEFAULT_FLAG_5)
    {
      settings.myflags |= BITFLAG_5;
    }
    if (DEFAULT_FLAG_6)
    {
      settings.myflags |= BITFLAG_6;
    }
    if (DEFAULT_FLAG_7)
    {
      settings.myflags |= BITFLAG_7;
    }
...
// Reads startup line from EEPROM. Updated pointed line string data.$W
